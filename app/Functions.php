...
function UploadFileRename($file_name) 
{
    $file_name = ucwords($file_name);

    $file_name = str_replace(' ', '-', $file_name); // Replaces all spaces with hyphens.

    $file_name = preg_replace('/[^A-Za-z0-9\-]/', '', $file_name); // Removes special chars.

    $file_name = $file_name."-".random_str('alpha', 4);
...
function sendHtmlMail($params) {
    ...
    \Mail::send('emails.index', $params, function($message) use ($params, $files) {
        $message->from('london@emfisecurities.com', 'EmfiSecurities');
